...
export default function ManageEventNewsPage() {
  ...
  const fetchEvents = async () => {
    if (!major) return;
    const q = query(eventsRef, where("major", "==", major));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        ...
        await updateDoc(eventDoc, { title, content, major });
        setEditingId(null);
        alert("글 수정이 완료되었습니다.");
      } else {
    ...
    <div style={styles.container}>
      <div style={styles.card}>
        ...
        <ul style={styles.list}>
          {events.map((event) => (
            <li key={event.id} style={styles.listItem}>
              <div style={styles.listHeader}>
                <strong style={styles.benefitTitle}>{event.title}</strong>
                <small style={styles.dateText}>
                  {formatDate(event.createdAt)}
                </small>
              </div>
              <p style={styles.benefitContent}>{event.content}</p>
          ...
          {events.length === 0 && <li>작성된 행사가 없습니다.</li>}
        </ul>
      </div>
    </div>
  );
